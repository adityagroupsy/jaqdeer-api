...
api.get_company_progress = (req, res) => {
  ...
  models.Setting.findAll({
    ...
    let sold_units = "";
    const promiss1_1 = get_page_customer_title(8, language_code).then(
      (return_titles) => {
...
api.check_mobile = (req, res) => {
  if (!req.body)
    return res.status(452).send({ msg: "missing_data", status: "452" });
  ...
  models.User.findOne({ where: { mobile: mobile, is_registered: 1 } })
    .then((user1) => {
      if (user1) {
...
api.check_email = (req, res) => {
  ...
  models.User.findOne({ where: { email: email, is_registered: 1 } })
    .then((user1) => {
      if (user1) {
...
api.register = (req, res) => {
  ...
  if (!["android", "ios"].includes(platform))
    return res.status(453).send({ msg: "invalid_platform", status: "453" });
        ...
        models.User.findOne({ where: { email: email, is_registered: 1 } })
          .then((user2) => {
            if (user2 && email != "") {
              ...
              if (customer_code == "") {
                models.User.findOne({ where: { mobile: mobile } })
                  .then((user1) => {
                    if (user1) {
                      ...
                      models.User.findOne({ where: { email: email } })
                        .then((user2) => {
                          if (user2 && email != "") {
                            msg =
                              langString[language_code]
                                .register_missing_customer;
                            return res
                              .status(455)
                              .send({ msg: msg, status: "462" });
                          } else {
                ...
                models.CustomerCode.findOne({
                  where: {
                    is_publish: 1,
                    is_used: { [Op.ne]: 1 },
                    code: { [Op.like]: customer_code },
                  },
                })
                  .then((codeCheck) => {
                    if (!codeCheck) {
...
api.refresh_fcm_token = (req, res) => {
  ...
  models.Device.findOrCreate({
    ...
    defaults: { device_id: device_id, user_id: user.id, user_model: 1 },
  })
    .spread((user_device, created) => {
      if (user_device) {
...
api.send_fcm_notification = (req, res) => {
  ...
  if (!check_attributes)
    return res.status(452).send({ msg: "missing_data", status: "452" });
  models.Device.findAll({ where: { register_token: { [Op.in]: user_tokens } } })
    .then((devices) => {
      const promises = [];
      if (devices.length > 0) {
...
api.logout = async (req, res) => {
  ...
  if (removed_tolken) {
    models.Device.destroy({
      where: { device_id: device_id, user_id: user.id, user_model: 1 },
    })
      .then((removed) => {
      ...
      if (!user) {
        ...
        models.VerifyCode.update(
          ...
          .then((data) => {
            ...
            const promise = models.VerifyCode.new_code(
              user.id,
              1,
              2,
              mail_code
            ...
            Promise.all(promises)
              .then((results) => {
                ...
                return res
                  .status(200)
                  .send({ msg: msg, status: "200", mail_code: mail_code });
              })
              .catch((error) => {
                return res
                  .status(500)
        ...
        models.VerifyCode.findOne({
          ...
          .then((forgot_password) => {
            if (forgot_password) {
              ...
              let diff = date
                .subtract(new Date(cairoNow), new Date(invalid_date))
                .toMinutes();
...
api.update_user_data = (req, res) => {
  ...
  /* let mobilevalidator               = require("phone");
    ...
    */
  models.User.findOne({
    where: { email: email, is_registered: 1, id: { [Op.ne]: user.id } },
  })
    .then((user1) => {
      if (user1) {
        msg = langString[language_code].register_exist_email;
        ...
        models.User.findOne({
          where: { mobile: mobile, is_registered: 1, id: { [Op.ne]: user.id } },
        })
          .then((user1) => {
            if (user1) {
              msg = langString[language_code].register_exist_mobile;
              ...
              if (password != "") {
                if (!old_password)
                  return res
                    .status(452)
                    .send({ msg: "missing_data", status: "452" });
                let user_pass = user.password;
                user_pass = user_pass.replace("$2y$", "$2b$");
                const bcrypt = require("bcryptjs");
                        ...
                        .then((hashed) => {
                          ...
                          user
                            ...
                            .then((savedUser) => {
                              ...
                              return res
                                .status(200)
                                .send({ msg: msg, status: "200", data: user });
                            })
                            .catch((error) => {
                              return res
                                .status(500)
...
api.list_showrooms = (req, res) => {
  ...
  models.Shoowroom.findAll({
    ...
    include: [
      {
        model: models.City,
        as: "city",
        attributes: [["title_" + language_code, "name"]],
      },
    ],
    offset: offset,
    limit: take,
    ...
    .then((showrooms) => {
      return res
        .status(200)
        .send({ msg: "done", status: "200", data: showrooms });
    })
    .catch((error) => {
      return res.status(500).send({ msg: error.message, status: "500" });
    });
...
api.list_magazines = (req, res) => {
  ...
  models.Magazine.findAll({
    where: { is_publish: 1 },
    attributes: [["title_" + language_code, "title"], "id", "image", "file"],
    offset: offset,
    limit: take,
    order: [["id", "DESC"]],
  })
...
api.list_projects = (req, res) => {
  ...
  if (!["1", "2"].includes(status))
    return res.status(453).send({ msg: "invalid_status", status: "453" });
  take = parseInt(take);
  offset = parseInt(offset);
  models.Project.findAll({
    ...
    .then((projects) => {
      ...
      projects.forEach((project) => {
        const promise1 = models.WishList.getIsFavorits(
          project.id,
          user,
          1
        ).then((user_fav) => {
      ...
      Promise.all(promises)
        .then((results) => {
          return res
            .status(200)
            .send({ msg: "done", status: "200", data: projects });
        })
        .catch((error) => {
          return res.status(500).send({ msg: error.message, status: "500" });
        });
...
api.get_project_by_id = (req, res) => {
  ...
  models.Project.findOne({
    ...
    .then((project) => {
      ...
      const promises = [];
      if (project) {
        const promise1 = models.Project.get_project_data(
          project.id,
          language_code
        ).then((project_data) => {
        ...
        const promise2 = models.WishList.getIsFavorits(
          project.id,
          user,
          1
        ).then((user_fav) => {
        ...
        .then((results) => {
          ...
          Promise.all(promises2)
            .then((results) => {
              return res
                .status(200)
                .send({ msg: "done", status: "200", data: project });
            })
            .catch((error) => {
              return res
                .status(500)
  ...
  if (user) {
    ...
    models.Slider.findAll({
      where: {
        is_publish: 1,
        [Op.or]: [
          { finished_at: { [Op.gte]: new Date() } },
          { finished_at: null },
        ],
        object_type: { [Op.notIn]: [8] },
      },
      attributes: [
        ["title_" + language_code, "title"],
      ...
      .then((sliders) => {
        return res
          .status(200)
          .send({ msg: "done", status: "200", data: sliders });
      })
      .catch((error) => {
        return res.status(500).send({ msg: error.message, status: "500" });
      });
...
api.get_page_title = (req, res) => {
  ...
  if (!check_attributes)
    return res.status(452).send({ msg: "missing_data", status: "452" });
  if (!["1", "2", "3", "4", "5", "6", "7", "8", "9"].includes(page_id))
    return res.status(453).send({ msg: "invalid_page", status: "453" });
  models.PageTitle.findAll({
    where: { page_id: page_id },
    attributes: ["key", ["title_" + language_code, "name"]],
  })
    .then((sliders) => {
      return res
        .status(200)
...
api.list_cities = (req, res) => {
  ...
  models.City.findAll({
    where: { is_publish: 1 },
    attributes: [["title_" + language_code, "name"], "id"],
    limit: 10,
  })
    .then((cities) => {
...
api.list_subjects = (req, res) => {
  ...
  models.Subject.findAll({
    ...
    .then((subjects) => {
      return res
        .status(200)
        .send({ msg: "done", status: "200", data: subjects });
    })
    .catch((error) => {
      return res.status(500).send({ msg: error.message, status: "500" });
    });
    ...
    .then((about) => {
      ...
      if (about) {
        for (let i = 0, len = about.length; i < len; i++) {
          ...
          if (
            ["recruitment_description", "recruitment_title"].includes(
              about[i].key
            )
          ) {
            if (language_code == "ar") {
  ...
  models.Recruitment.create({
    ...
    .then((recruitment) => {
      if (cv_file) {
        ...
        const image_name =
          mobile + "-" + job_id + generateRandomInt(4) + "." + type;
        const image_path =
          "../public_html/jadeertest/public/images/cvs/" + image_name;
    ...
    .then((articles) => {
      ...
      if (articles.length > 0) {
        articles.forEach((article) => {
          const promise1 = formatDateHuman(
      ...
      Promise.all(promises)
        .then((results) => {
          return res
            .status(200)
            .send({ msg: "done", status: "200", data: articles });
        })
        .catch((error) => {
          return res.status(500).send({ msg: error.message, status: "500" });
        });
    ...
    .then((article) => {
      ...
      Promise.all(promises)
        .then((results) => {
          ...
          return res
            .status(200)
            .send({ msg: "done", status: "200", data: article });
        })
        .catch((error) => {
          return res.status(500).send({ msg: error.message, status: "500" });
        });
    ...
    .then((media_centers) => {
      ...
      Promise.all(promises)
        .then((results) => {
          return res
            .status(200)
            .send({ msg: "done", status: "200", data: media_centers });
        })
        .catch((error) => {
          return res.status(500).send({ msg: error.message, status: "500" });
        });
...
api.import_contact = (req, res) => {
  ...
  if (!contacts.isArray)
    return res
      .status(481)
      .send({ msg: "contacts_must_be_array", status: "481" });

  msg = langString[language_code].max_contact_30;
  if (contacts.length > 30)
    return res.status(482).send({ msg: msg, status: "482" });
  ...
  contacts.forEach((contact) => {
    if (contact.mobile != "") {
      ...
      const promise1 = create_new_contact(contact_obj).then(
        (created_row) => {}
      );
      promises.push(promise1);
    }
  ...
  Promise.all(promises)
    .then((results) => {
      ...
      return res
        .status(200)
        .send({ msg: msg, status: "200", data: { msg: msg } });
    })
    .catch((error) => {
      return res.status(500).send({ msg: error.message, status: "500" });
    });
...
api.get_user_progress = (req, res) => {
  ...
  models.UserUnit.findAll({
    ...
    .then((user_units) => {
      ...
      const promiss1_1 = get_page_customer_title(7, language_code).then(
        (return_titles) => {
          titles = return_titles;
          if (titles.length > 0) {
            titles.forEach((title) => {
        ...
        .then((results) => {
          ...
          if (user_units.length > 0) {
            user_units.forEach((user_unit) => {
              ...
              const promise1 = get_project_name(
                user_unit.project_id,
                language_code
              ).then((name) => {
                unit.name = name;
              });
              promises.push(promise1);
              const promise2 = get_unit_type(
                user_unit.unit_id,
                language_code
              ).then((type) => {
                unit.type = type;
              ...
              promises.push(promise2);
              const promise3 = get_no_paid_payments(user_unit.id, user.id).then(
                (no_paid_payments) => {
                  unit.no_paid_payments = no_paid_payments;
                }
              );
              promises.push(promise3);
              const promise4 = get_all_payments(user_unit.id, user.id).then(
                (all_payments) => {
            ...
            .then((results) => {
              if (progress.length > 0) {
                progress.forEach((unit) => {
                  ...
                  unit.progress = unit.progress.toString();
                  if (language_code == "ar") {
                    unit.no_paid_payments =
                      unit.no_paid_payments.toArabicDigits();
              ...
              return res
                .status(200)
                .send({ msg: "done", status: "200", data: progress });
            })
            .catch((error) => {
              return res
                .status(500)
  ...
  models.UserUnit.findAll({
    ...
    .then((data) => {
      ...
      data.forEach((project) => {
        const promise1 = models.Payment.getRemainPayment(
          project.id,
          project.total_price
        ).then((remain_payment) => {
      ...
      Promise.all(promises)
        .then((results) => {
          return res
            .status(200)
            .send({ msg: "done", status: "200", data: data });
        })
        .catch((error) => {
          return res.status(500).send({ msg: error.message, status: "500" });
        });
    ...
    .then((projects) => {
      if (projects.length > 0) {
        projects.forEach((user_unit) => {
          ...
          if (!user_unit.project.progress_text)
            user_unit.project.progress_text = "";
          if (!user_unit.project.progress_percentage)
            user_unit.project.progress_percentage = 0;
          ...
          /*   if(  user_unit.project.image )
                ...
                */
          if (language_code == "ar") {
            if (!user_unit.unit.area) {
              user_unit.unit.area = "";
            } else {
              user_unit.unit.area = user_unit.unit.area.toArabicDigits();
            }
          } else {
            if (!user_unit.unit.area) {
              user_unit.unit.area = "";
      ...
      models.PageTitle.findAll({
        ...
        .then((titles) => {
          const promises = [];
          projects.forEach((project) => {
            const promise3 = models.Payment.getRemainPayment(
              project.id,
              project.total_price
            ).then((remain_payment) => {
          ...
          Promise.all(promises)
            .then((results) => {
              return res.status(200).send({
                msg: "done",
                status: "200",
                data: { projects: projects, titles: titles },
              });
            })
            .catch((error) => {
              return res
      ...
      if (history.length > 0) {
        history.forEach((history_obj) => {
          ...
          if (history_obj.due_date != null) {
            if (language_code == "ar") {
              history_obj.duedate = date.format(
                new Date(history_obj.due_date),
                "YY/MM/DD"
              );
          ...
          if (history_obj.date != null && history_obj.date != "-") {
            if (language_code == "ar") {
              history_obj.payment_date = date.format(
                new Date(history_obj.date),
                "YY/MM/DD"
              );
              history_obj.payment_date =
                history_obj.payment_date.toArabicDigits();
            } else {
              history_obj.payment_date = date.format(
                new Date(history_obj.date),
                "DD/MM/YY"
          ...
          if (
            history_obj.status == 0 &&
            new Date().getTime() > new Date(history_obj.due_date).getTime()
          ) {
            const promise1 = translate_payment_status(4, language_code).then(
              (formated_status) => {
            ...
            const promise = translate_payment_status(
              history_obj.status,
              language_code
            ).then((formated_status) => {
              history_obj.payment_status = formated_status;
            });
            promises.push(promise);
          }
              ...
              next_payments.forEach((next_payment) => {
                if (next_payment) {
                  ...
                  if (next_payment.due_date != null) {
                    if (language_code == "ar") {
                      next_payment.duedate = date.format(
                        new Date(next_payment.due_date),
                        "YY/MM/DD"
                      );
                      next_payment.duedate =
                        next_payment.duedate.toArabicDigits();
                    } else {
                      next_payment.duedate = date.format(
                        new Date(next_payment.due_date),
                        "DD/MM/YY"
                  ...
                  if (next_payment.date != null) {
                    if (language_code == "ar") {
                      next_payment.payment_date = date.format(
                        new Date(next_payment.date),
                        "YY/MM/DD"
                      );
                      next_payment.payment_date =
                        next_payment.payment_date.toArabicDigits();
                    } else {
                      next_payment.payment_date = date.format(
                        new Date(next_payment.date),
                        "DD/MM/YY"
                  ...
                  if (
                    next_payment.status == 0 &&
                    new Date().getTime() >
                      new Date(next_payment.due_date).getTime()
                  ) {
                    const promise1 = translate_payment_status(
                      4,
                      language_code
                    ).then((formated_status) => {
              ...
              Promise.all(promises1)
                .then((results) => {
                  return res.status(200).send({
                    msg: "done",
                    status: "200",
                    data: { history: history, next_payment: next_payments },
                  });
                })
                .catch((error) => {
                  return res
    ...
    .then((decore_type) => {
      ...
      return res.status(200).send({
        ...
        data: {
          msg: msg,
          price: price,
          meter_price: parseFloat(decore_type.price),
        },
      });
    })
    .catch((error) => {
      ...
      models.Setting.findAll({
        ...
        .then((about) => {
          ...
          return res.status(200).send({
            ...
            data: {
              subjects: subjects,
              decoration_number: data["decoration_number"],
            },
          });
        })
        .catch((error) => {
    ...
    .then((favorites) => {
      ...
      favorites.forEach((object_data) => {
        if (object_data.object_type == 1) {
          // project
          const promise1 = models.Project.getProductData(
          ...
          const promise1 = models.Unit.getUnitData(
            object_data.object_id,
            language_code
          ).then((user_fav) => {
            if (user_fav) {
        ...
        .then((results) => {
          ...
          wishLists.forEach((object_data) => {
            if (object_data.object_type == 2) {
              // project
              const promise2 = models.Project.getProductData(
                object_data.project_id,
                language_code
              ).then((project) => {
                if (project) {
          ...
          Promise.all(promises2)
            .then((results) => {
              return res
                .status(200)
                .send({ msg: "done", status: "200", data: wishLists });
            })
            .catch((error) => {
              return res
                .status(500)
        ...
        .then((payment) => {
          if (payment == null) {
            ...
            if (payment.due_date != null) {
              ...
              let days = date
                .subtract(new Date(due_date), new Date(cairoNow))
                .toDays();
              // resolve(days+2);
              resolve(days);
            }
