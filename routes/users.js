...
apiRoutes.post('/check_forget_password_code' 				, controllers.check_forget_password_code);
apiRoutes.post('/send_forgot_password_code'	 				, controllers.send_forgot_password_code);
...
apiRoutes.use(async function(req, res, next) {
  ...
  // check header or url parameters or post parameters for token  
	...
	if (token) {
	    ...
	    let decoded = await jwtr.verify(token, configJwt.secret)
	    .then(decoded =>{
			...
			if(decoded == undefined){
		        ...
		        models.User.findOne({  where:{ id: decoded.user } } ).then( user => {
					if (! user){
						req.user            = null; 
						next();
